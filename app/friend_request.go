...
func (a *App) SendFriendRequest(senderid string, receiverid string) *model.AppError{
	...
	if(request != nil){
		return model.NewAppError("SendFriendRequest", "app.friend_request.already_in_relation", nil, err.Error(), http.StatusInternalServerError)
	}
	...
	friend_request, err = a.Srv().Store.FriendRequest().Save(friend_request)
	if(err!=nil){
